...
namespace Alpha.Collector.Core
{
    ...
    /// 广西11选5采集器管理器
    /// </summary>
    public class GX11X5PickerManager : PickerManager
    {
        /// <summary>
        /// 获取采集器列表
        /// </summary>
        ...
        {
            List<Type> list = ReflectionHelper.GetClasses<IGX11X5Picker>();
            List<IPicker> pickerList = new List<IPicker>();
            foreach (Type type in list)
            {
                try
                {
                    IGX11X5Picker picker = Activator.CreateInstance(type) as IGX11X5Picker;
