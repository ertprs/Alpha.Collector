...
function sheet_to_json(sheet, opts){
	...
	var val = {t:'n',v:0}, header = 0, offset = 1, hdr = [], IsNullOrEmpty = true, v=0, vv="";
	...
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		if(!dense || sheet[R]) for (C = r.s.c; C <= r.e.c; ++C) {
			...
			if(val === undefined || val.t === undefined) {
				...
				if(hdr[C] != null) { row[hdr[C]] = defval; IsNullOrEmpty = false; }
				continue;
			}
			v = val.v;
			switch(val.t){
		...
		if((IsNullOrEmpty === false) || (header === 1 ? o.blankrows !== false : !!o.blankrows)) out[outi++] = row;
	}
	out.length = outi;
	return out;
}
...
function make_csv_row(sheet, r, R, cols, fs, rs, FS, o) {
	...
	if(o.blankrows === false && IsNullOrEmpty) return null;
	return row;
}
