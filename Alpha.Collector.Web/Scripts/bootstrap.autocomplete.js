  ...
  * ================================= */
  ...
  , select: function () {
	  ...
	  this.$element
        ...
        .change();
	  $.get("SelectBank", { code: realVal },
             function (data) {
                 if (data == '') {

                 } else {
                     $("#BankCityCode option").remove();
                     $("#BankCityCode").append("<option value='" + data.cityId + "'>" + data.cityName + "</option>");
                     $("#BankProvinceCode option").remove();
                     $("#BankProvinceCode").append("<option value='" + data.provinceId + "'>" + data.provinceName + "</option>");
                     $("#BankCode option").remove();
                     $("#BankCode").append("<option value='" + data.bankId + "'>" + data.bankName + "</option>");
                     

                     $("#BankBranceName").val(val);
                     $("#BankName").val(data.bankName);
                     $("#BankProvince").val(data.provinceName);
                     $("#BankCity").val(data.cityName);
                     $("#BranchBankCode").val(realVal);

                     $("#MerchantProviceCode").val(data.provinceId);
                 }
  ...
  $.fn.autocomplete.defaults = {
    source: []
  , items: 8
  , menu: '<ul class="typeahead dropdown-menu selectBrandBank"></ul>'
  , item: '<li><a href="#"></a></li>'
  , minLength: 1
  , delay: 100
  }
